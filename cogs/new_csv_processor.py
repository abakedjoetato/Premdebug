...
class CSVProcessorCog(commands.Cog):
    ...
    async def process_csv_files_task(self):
        ...
        try:
            ...
            for server_id, config in server_configs.items():
                logger.info(f"Processing CSV files for server {server_id}")
    ...
    async def _process_server_config(self, server: Dict[str, Any], server_id: str, 
        ...
        if sftp_config["hostname"] and sftp_config["username"] and sftp_config["password"]:
    ...
    async def process_csv_command(
        ...
        try:
            ...
            await interaction.followup.send(f"Error processing CSV files: {str(e)}")
    ...
    async def historical_parse_command(
        ...
        try:
            ...
            await interaction.followup.send(f"Error processing historical data: {str(e)}")
    ...
    async def csv_status_command(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(f"Error retrieving CSV processor status: {str(e)}")
