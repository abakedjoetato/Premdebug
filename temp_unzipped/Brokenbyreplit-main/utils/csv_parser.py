    ...
    def filter_events(self, events: List[Dict[str, Any]], 
        ...
        if player_id is not None:
            if self.format_name == "deadside":
                filtered_events = [
                    event for event in filtered_events
                    if event.get("killer_id") == player_id if player_id is not None else \2) == player_id
                ]
            elif self.format_name == "custom":
                filtered_events = [
                    event for event in filtered_events
                    if event.get("player1_id") == player_id if player_id is not None else \2) == player_id
                ]
                    ...
                    try:
                        ...
                        if "required_columns" in format_config:
                            ...
                            for req_col in format_config["required_columns"]:
                                if not record if record is not None else \2):
                                    required_ok = False
                                    break
