...
async def send_safely(ctx, embed_coroutine, ephemeral=False, **kwargs):
        """
        Safely sends an embed by properly awaiting it first.
        This helps prevent 'coroutine' object has no attribute 'to_dict' errors.
        
        Args:
            ctx: The command context or interaction
            embed_coroutine: A coroutine that returns a Discord Embed
            ephemeral: Whether message should be ephemeral (for interactions)
            **kwargs: Additional kwargs to pass to send()
        ...
        try:
            ...
            if inspect.iscoroutine(embed_coroutine):
                embed = await embed_coroutine
            else:
                embed = embed_coroutine
                
            # Handle different context types
            if hasattr(ctx, 'response') and hasattr(ctx.response, 'send_message'):
                # It's an interaction
                if not ctx.response.is_done():
                    return await ctx.response.send_message(embed=embed, ephemeral=ephemeral, **kwargs)
                else:
                    return await ctx.followup.send(embed=embed, ephemeral=ephemeral, **kwargs)
            else:
            ...
            logger.error(f"Error in send_safely: {e}")
            
            # Create and send a fallback embed
            import discord
            from datetime import datetime
            
            fallback = discord.Embed(
                title="Error Sending Embed",
                description="There was an error sending the requested embed.",
                color=0xFF0000,
                timestamp=datetime.utcnow()
            )
            fallback.set_footer(text="Please report this error to the bot developers")
            
            try:
                if hasattr(ctx, 'response') and hasattr(ctx.response, 'send_message'):
                    if not ctx.response.is_done():
                        return await ctx.response.send_message(embed=fallback, ephemeral=True)
                    else:
                        return await ctx.followup.send(embed=fallback, ephemeral=True)
                else:
                    return await ctx.send(embed=fallback)
            except:
                logger.error("Failed to send fallback embed")
                return None
...
class EmbedBuilder:
    ...
    async def create_server_stats_embed(cls, 
        ...
        return await cls.create_embed(
            ...
            color=color,
            fields=fields,
            thumbnail_url=server_icon if server_icon is not None else \2,
            footer_text="Last updated",
            timestamp=datetime.utcnow(),
            guild=guild,
            bot=bot
...
async def ensure_embed_ready(embed_or_coroutine):
    """
    Utility to ensure an embed is properly awaited if it's a coroutine.
    Helps prevent 'coroutine' object has no attribute 'to_dict' errors.
    
    Args:
        embed_or_coroutine: Either a Discord Embed or a coroutine that returns one
        
    Returns:
        Discord Embed object that's ready to use
    """
    import inspect
    
    # If it's a coroutine, await it
    if inspect.iscoroutine(embed_or_coroutine):
        try:
            return await embed_or_coroutine
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            ...
            fallback = discord.Embed(
                ...
                description="There was an error creating the requested embed.",
                color=0xFF0000,
                timestamp=datetime.utcnow()
            )
            fallback.set_footer(text="Please report this error to the bot developers")
    ...
    # If it's already an embed, return it directly
    return embed_or_coroutine
