...
class Player(BaseModel):
    ...
    async def update_stats(
        ...
        try:
            ...
            try:
                kills_inc = max(0, int(kills or 0))
                deaths_inc = max(0, int(deaths or 0))
                suicides_inc = max(0, int(suicides or 0))
            except (ValueError, TypeError):
                logger.error(
    ...
    def from_document(
        ...
        if "player_id" not in doc or not doc["player_id"]:
        ...
        try:
            ...
            if doc["name"] not in known_aliases:
    ...
    async def create_or_update(
        ...
        try:
            ...
            # Check if player exists using a properly constructed query
